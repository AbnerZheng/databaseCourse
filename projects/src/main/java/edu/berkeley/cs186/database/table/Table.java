...
public class Table implements Iterable<Record>, Closeable {
    ...
    public RecordID addRecord(List<DataType> values) throws DatabaseException {
        try {
            ...
            if (!freePages.isEmpty()) {
                int p = freePages.first();
                page = this.allocator.fetchPage(p); //获取空闲页
            } else { //freePage为空时,需要新建一个page
                page = this.allocator.fetchPage(this.allocator.allocPage());
                isNew = true;
            }
            byte[] header = this.readPageHeader(page); //读取头
            int i = 0;
            int slotIndex = 0;
            while (i < pageHeaderSize) {
                if (header[i] != (byte) 0xFF) { //如果有空余slot
                    int bitOffset;
                    for (slotIndex = 0; slotIndex < 8; ++slotIndex) {
                        bitOffset = 7 - slotIndex;
                        byte mask = (byte) (1 << bitOffset);
                        byte value = (byte) (header[i] & mask);
                        if (value == 0) { //找到空slot
                            entryNum = i * 8 + slotIndex;
                            break;
                        }
                    }
                    break;
            ...
            if (entryNum == -1) {
                throw new DatabaseException("There is some error in freePages maintaining");
            }
            int entrySize = this.schema.getEntrySize();
            int offset = this.pageHeaderSize + (entrySize * entryNum);
            page.writeBytes(offset, entrySize, this.schema.encode(record));
            this.writeBitToHeader(page, entryNum, (byte) 0x1);
            this.stats.addRecord(record);
            this.numRecords++;

            // 更新freePages
            if (spaceOnPage(page) && isNew) {
                freePages.add(page.getPageNum());
            }
            if (!spaceOnPage(page) && !isNew) {
                freePages.remove(page.getPageNum());
            }
    ...
    public Record updateRecord(List<DataType> values, RecordID rid) throws DatabaseException {
        try {
            ...
            throw new DatabaseException("The values do not correspond to the schema of this table");
        }
    }
    ...
    private void setEntryCounts() {
        ...
        int maxSize = Page.pageSize / entrySize;
        maxSize = maxSize & 0xfffffff8; // multiple of 8
        while (maxSize * entrySize + maxSize / 8 > Page.pageSize) {
            maxSize -= 8;
        }
        numEntriesPerPage = maxSize;
    ...
    private int numValidEntries(Page p) {
        ...
        //统计b二进制表示中1的个数
        //优化,利用公式countBit(n & (n - 1))+1 === countBit(n)
        onesOfbytes[0] = 0;
        for (int i = 1; i < onesOfbytes.length; ++i) {
